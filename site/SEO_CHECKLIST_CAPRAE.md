...
|---|---|---|
| `<title>` (unique per page, ≤ 60 chars) | Present — unique per page (home, /about/, /services/ + 6 service pages, /how-it-works/, /results/, /pricing/, /contact/, /team/ + 7 profiles, /case-study/, /faq/, legal), set in `src/build_pages.py` / `build_legal_pages.py`. Two supplied service titles are 62 chars | High. Truncated in SERPs; every page shows the same snippet. |
| `<meta name="description">` (≤ 155 chars, unique) | Present — unique per page; Entrepreneurship (163) exceeds 155. Team profile pages have none yet | High. Directly affects CTR from search results. |
...
| `sitemap.xml` | Present — every page, including /about/, /team/ + profiles, /case-study/, /faq/ and legal. Generated by `src/build_pages.py` | Critical. Should list only canonical URLs of this site. |
...
| `ProfessionalService` + `Service` + `Offer` / `OfferCatalog` | Present — service/offer graph on every page, plus a `Service` block on each /services/<slug>/ and /services/branding/ page | High. Service rich results; strong entity signal. |
| `Article` schema on Field Notes posts | Present — `Article` + `ImageObject` + `WebPage` per post | Medium. Article rich results. |
| `BreadcrumbList` schema | Present — per page, e.g. Home › Services › AI-Readiness (not on home) | Medium. Rich breadcrumb display in SERPs. |
| `FAQPage` schema on service pages | Absent — /faq/ is a "coming soon" placeholder; add the schema with the Q&As | Medium. FAQ rich results. |
...
| URL slugs (readable, hyphenated) | Present — readable, lowercase, hyphenated (`/services/ai-readiness/`, `/how-it-works/`); Field Notes posts too | Medium. |
...
- **Critical (do now):** Confirm Google Search Console verification (`googled90b83ade46def6e.html`) is the correct token for this property, deploy, and submit the sitemap. (Field Notes is **not** a services page — it mirrors from partners and canonicalizes there; leave it out of this sitemap.)
...
- **Medium (later):** confirm `WebSite` `SearchAction`, Bing Webmaster verification. (`BreadcrumbList`, 404 page, and server logs now done; `FAQPage` still absent.)
