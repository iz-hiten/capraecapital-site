...
|---|---|---|
| `<title>` (unique per page, ≤ 60 chars) | Present — unique per page (home, /services/, 6 service pages, /how-it-works/, /results/, /pricing/, /contact/), set in `src/build_pages.py`. Two supplied service titles are 62 chars | High. Truncated in SERPs; every page shows the same snippet. |
| `<meta name="description">` (≤ 155 chars, unique) | Present — unique per page, set in `src/build_pages.py`; home (179) and Entrepreneurship (163) exceed 155 | High. Directly affects CTR from search results. |
...
| `sitemap.xml` | Present — every page (home, /services/ + 6 service pages, /how-it-works/, /results/, /pricing/, /contact/, legal). Generated by `src/build_pages.py` | Critical. Should list only canonical URLs of this site. |
...
| `FAQPage` schema on service pages | Absent — no FAQ section on the site (as of 2026-09-24) | Medium. FAQ rich results. |
